const init = () => {
    document.querySelector(".rules__openButton").addEventListener("click", showRules);
    document.querySelectorAll(".options__button--choose").forEach((button) => {
        button.addEventListener("click", changeToStep2);
    });
    document.querySelector(".resultTable__button").addEventListener("click", resetGame);
    showScore();
};
...
const removePickedClass = () => {
    const playerBorderElement = document.querySelector(".options__button--pickByPlayerPosition");
    playerBorderElement.classList.remove(`options__button--background${game.playerPick}`);
    const aiBorderElement = document.querySelector(".options__button--pickByAiPosition");
    aiBorderElement.classList.remove(`options__button--background${game.aiPick}`);
}

const showScore = () => {
    // let scorePoints = player.wins - player.loses;
    let scorePoints = Number(localStorage.getItem("wins")) - Number(localStorage.getItem("loses"));
...
const setPlayerPick = (picked) => {
    ...
    const playerImgElement = document.querySelector(".options__bigSizeButtonImg--player");
...
const setAiPick = () => {
    ...
    const aiImgElement = document.querySelector(".options__bigSizeButtonImg--ai");
